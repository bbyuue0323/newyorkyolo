...
interface Member {
  ...
  role: string;
  bio: string;
  sns?: { instagram?: string; tiktok?: string; youtube?: string; threads?: string };
}
...
export default function Members() {
  ...
  const [tooltipPos, setTooltipPos] = useState<TooltipPos>({ left: '50%', transform: 'translateX(-50%)', arrowLeft: '50%' });
  ...
  const calcPos = (index: number): TooltipPos => {
    ...
    if (!el) return { left: '50%', transform: 'translateX(-50%)', arrowLeft: '50%' };
    ...
    if (tooltipLeft + TOOLTIP_WIDTH > vw - MARGIN) tooltipLeft = vw - MARGIN - TOOLTIP_WIDTH;

    const offsetFromEl = tooltipLeft - rect.left;
    const arrowCenter = rect.width / 2 - offsetFromEl;
    const arrowPct = Math.min(Math.max((arrowCenter / TOOLTIP_WIDTH) * 100, 10), 90);
  ...
  const members: Member[] = [
    {
      name: '國嶋賢志朗',
      image: 'https://static.readdy.ai/image/0c61843cac5595a4ea86012b4ca98e8d/fadbdb3d937ca2482edc86d863fa2672.jpeg',
      objectPosition: 'center 10%',
      role: 'Player',
      bio: '夢を追いかけ、人生の可能性を広げるためにニューヨークへ。現在はNYの不動産企業で働いています。「死ぬこと以外はかすり傷」。挑戦することを前提に、人生を設計しています。',
      sns: {
        instagram: 'https://www.instagram.com/kenshiro_newyorkyolo?igsh=MWtwNnp3ZTR2amFlNw==',
    ...
    {
      name: '山本竜也',
      image: 'https://static.readdy.ai/image/0c61843cac5595a4ea86012b4ca98e8d/26b2ffa31d1d197c628828625372d29f.jpeg',
      ...
      <div className="max-w-7xl mx-auto px-6">
        <ScrollReveal>
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-white mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>メンバー</h2>
              ...
              >
                ...
                <div
                  ...
                  style={{
                    ...
                    transform: hoveredIndex === index ? tooltipPos.transform : 'translateX(-50%)',
                  }}
                >
                  <div
                  ...
                  >
                    {member.role && (
                      <p className="text-[#B11226] text-xs font-semibold tracking-widest uppercase mb-2">{member.role}</p>
                    )}
                    <p className="text-gray-300 text-sm leading-relaxed mb-4">{member.bio}</p>
                    {member.sns && Object.keys(member.sns).length > 0 && (
                      <div className="flex gap-2 pointer-events-auto">
                        {member.sns.instagram && (
                          <a href={member.sns.instagram} target="_blank" rel="noopener noreferrer"
                        ...
                        {member.sns.tiktok && (
                          <a href={member.sns.tiktok} target="_blank" rel="noopener noreferrer"
                            className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-[#B11226]/20 border border-white/10 hover:border-[#B11226]/50 text-gray-400 hover:text-[#B11226] transition-all duration-300 cursor-pointer">
                            <i className="ri-tiktok-line text-sm"></i>
                          </a>
                        )}
                        {member.sns.youtube && (
                          <a href={member.sns.youtube} target="_blank" rel="noopener noreferrer"
                            className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-[#B11226]/20 border border-white/10 hover:border-[#B11226]/50 text-gray-400 hover:text-[#B11226] transition-all duration-300 cursor-pointer">
                            <i className="ri-youtube-line text-sm"></i>
                          </a>
                        )}
                        {member.sns.threads && (
                          <a href={member.sns.threads} target="_blank" rel="noopener noreferrer"
                            className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-[#B11226]/20 border border-white/10 hover:border-[#B11226]/50 text-gray-400 hover:text-[#B11226] transition-all duration-300 cursor-pointer">
                  ...
                  <div
                    ...
                    style={{
                      left: hoveredIndex === index ? tooltipPos.arrowLeft : '50%',
                      bottom: '-8px',
                ...
                <h3 className="text-2xl font-bold text-white group-hover:text-[#B11226] transition-colors duration-300" style={{ fontFamily: "'Playfair Display', serif" }}>{member.name}</h3>
              </div>
            </ScrollReveal>
          ))}
        </div>
